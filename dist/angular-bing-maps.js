...
function pushpinDirective($compile) {
    'use strict';
    ...
    function link(scope, element, attrs, mapCtrl) {
        ...
        scope.$watch('options', function (newOptions) {
            newOptions.htmlContent = "<div class='pin " + newOptions.pinColorClass + "'><span class='pin-number'>";
            if (newOptions.pinData !== undefined) {
                newOptions.htmlContent += newOptions.pinData;
            }
            newOptions.htmlContent += "</span></div>";

            if (newOptions.showPulse) {
                $('.pin-pulse').remove();
                newOptions.htmlContent += "<div ng-if='facility.options.pinColorClass' class='pin-pulse'></div>";
            };
            ...
            angular.forEach(events, function(usersHandler, eventName) {
                ...
                var bingMapsHandler = Microsoft.Maps.Events.addHandler(scope.pin, eventName, function(event) {
                    ...
                    if (event.eventName == 'click') {
                        //change options here on click so options event gets triggered and pulse can chnage 
                        scope.options.triggerOptionChange = !scope.options.triggerOptionChange;
                    }
